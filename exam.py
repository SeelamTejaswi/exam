...
mydb=mysql.connector.connect(host='localhost',user='root',password='Admin',db='exams')
    ...
    if request.method == 'POST':
        ...
        for key in request.form:
            if key.startswith('question_'):
                ...
                cursor.execute('SELECT correctoption, marks FROM questions WHERE question_id=%s', (question_id,))
                correct_option, marks = cursor.fetchone()

                if selected_option == correct_option:
                    total_marks += marks

                cursor.execute('INSERT INTO submit (optionselected, studentid, course_id, question_id, marks, total_marks) VALUES (%s, %s, %s, %s, %s, %s)',
                               (selected_option, student_id, course_id, question_id, marks if selected_option == correct_option else 0, total_marks))
                mydb.commit()
